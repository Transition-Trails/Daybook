/**
 * Trend Research Studio — focus prompt → Claude returns 5 trend cards.
 * Each card has "→ Theme Studio" and "→ Pack Studio" quick-jump actions.
 */
import { useState } from "react";
import { useMutation } from "@tanstack/react-query";
import { useLocation } from "wouter";
import {
  Loader2, TrendingUp, Lightbulb, ArrowRight, AlertCircle, Info,
} from "lucide-react";
...
const SYSTEM_PROMPT = `You are a trend analyst for a premium digital planner brand called Daybook.
...
export default function TrendResearch() {
  ...
  const goToTheme = (idea: string) => {
    // Store in sessionStorage so ThemeStudio can read it
    sessionStorage.setItem("studioIdea", idea);
    navigate("/studios/theme");
  };
      ...
      {trends.length > 0 && !generate.isPending && (
        <div className="space-y-3">
          {trends.map((card, i) => (
            <Card
              key={i}
              className="group transition-shadow hover:shadow-md border-[#E7DCCB]"
            >
              <CardContent className="pt-5 pb-4">
                <div className="flex items-start gap-4">
                  ...
                  <div
                    ...
                    style={{
                      background: "linear-gradient(135deg, #C87560, #A85E4E)",
                      color: "#fff",
                    }}
                  >
                    {i + 1}
