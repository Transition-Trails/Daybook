...
// Before tracked migrations became the deployment path, the shared development
// database recorded these already-applied migrations as they were introduced,
// rather than in their final journal order. Keep this exact, known order here
// instead of accepting arbitrary reordering in the verifier.
...
function ledgerRowMatchesMigration(row, migration) {
  return row.hash === migration.hash && String(row.created_at) === String(migration.createdAt);
}

async function repairKnownOutOfOrderLedger(client, migrations) {
  ...
  if (legacyMigrations.some(migration => !migration)) {
    throw new Error(
      "Cannot safely repair the known legacy Drizzle ledger: its migration tags no longer "
      + "exist in the checked-in journal.",
    );
  }
...
async function prepareMigrationLedger() {
  ...
  try {
    ...
    const repairedKnownLedger = await repairKnownOutOfOrderLedger(client, migrations);
    await client.query("COMMIT");
