...
const READABLE_REQUEST = /\b(?:(?:readable|legible)\s+(?:text|wording|words?|writing|lettering|copy)|handwritten|typed|printed\s+(?:text|wording|words?|writing|lettering|copy|label|caption|title|name|legend|heading|inscription)|lettered|inscribed|captioned|named|dated|addressed|signed|initialed|postmark(?:ed)?|botanical identifications?|specimen (?:numbers?|data)|catalog(?:ue)? (?:data|fields?|entries)|correspondence prose|ledger (?:contents?|entries)|map labels?|geographic labels?|historical claims?|signatures?|initials?|institutional (?:names?|information)|label reads?|text reads?|wording reads?)\b/i;
