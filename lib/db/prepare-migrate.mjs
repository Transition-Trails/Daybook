...
const knownLegacyBaselineMismatches = new Set([
  // Wave 12 intentionally removes the legacy authority column before a
  // pre-ledger database is converted to tracked migration history.
  "missing column public.users.role",
...
async function getLegacySchemaFingerprint(client) {
  ...
  const repairableChangedColumns = new Set();
  for (const [key, expected] of expectedColumns) {
    ...
    if (!actual) {
      ...
      if (isRepairableLegacyColumnDifference(key, expected, actual)) {
        repairableChangedColumns.add(`changed column ${key}`);
      }
    }
  }
...
async function getAlreadyAppliedMigrationTags(client) {
  ...
  const existingPlannerConstraints = await hasConstraints(client, plannerInteriorConstraints);
  const existingPlannerIndexes = await hasIndexes(client, plannerInteriorIndexes);
  const hasPlannerTrigger = await hasTrigger(client, "planner_interior_versions_immutable_trigger");
...
async function prepareMigrationLedger() {
  ...
  try {
    ...
    const unknownMismatches = fingerprint.mismatches
      .filter(mismatch => (
        !knownLegacyBaselineMismatches.has(mismatch)
        && !fingerprint.repairableChangedColumns.has(mismatch)
      ));
