...
## The rule
`buildPdf` accepts a 9th string parameter `einkDevice` (preset key like `"remarkable"`). When set it automatically forces `inkFriendly=true`, overrides the page trim to the device's native dimensions, and enforces minimum 0.75 pt line weights. The colour/standard-trim PDF is still built as the main file; the e-ink variant (B&W + device trim) is uploaded as `inkFriendlyPdfFileId`.
...
## Page sizing
Inside `buildPdf`, when `einkDevice` is set the preset's `pts` dimensions replace `PAGE_SIZES[sizeKey]`. Portrait only for e-ink (no landscape override). The existing MARGIN=40 pt already covers safe inset for all four device toolbars.
