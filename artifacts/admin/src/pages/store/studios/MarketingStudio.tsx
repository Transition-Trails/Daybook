    ...
    <div className="flex h-[calc(100vh-8rem)] gap-0 -mx-8 -mt-4 overflow-hidden rounded-lg border border-border bg-background">
      ...
      <aside className="w-80 shrink-0 flex flex-col border-l border-border bg-card">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
          <div className="flex items-center gap-2">
            <Bot className="w-4 h-4 text-[#C87560]" />
            ...
            <div
              ...
              className={`text-xs rounded-lg px-3 py-2 leading-relaxed ${
                msg.role === "assistant"
                  ...
                  : "bg-[#C87560]/10 text-[#C87560] self-end"
              }`}
            >
              <CopilotText content={msg.content} storeId={storeId} />
            </div>
          ))}
          {isGenerating && (
        ...
        <div className="px-4 py-3 border-t border-border">
          <div className="flex gap-2">
            ...
            <Button
              size="icon"
              className="h-8 w-8 bg-[#C87560] hover:bg-[#A85E4E] text-white shrink-0"
              onClick={sendCopilot}
              disabled={!copilotInput.trim() || isGenerating}
            >
              <Send className="w-3.5 h-3.5" />
            </Button>
          </div>
          <p className="text-[10px] text-muted-foreground mt-1.5">
            Try: "draft listing", "draft social", "draft all"
          </p>
        </div>
      </aside>
    </div>
